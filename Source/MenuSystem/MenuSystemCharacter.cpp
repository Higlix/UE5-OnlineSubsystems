	...
	JoinSessionCompleteDelegate(FOnJoinSessionCompleteDelegate::CreateUObject(this, &ThisClass::OnJoinSessionComplete))
{
	...
	if (OnlineSubsystem)
	{
		this->OnlineSessionInterface = OnlineSubsystem->GetSessionInterface();
...
void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		...
		/* Join the lobby level and starting a listen server */
		UWorld* World = GetWorld();
		if (World)
		{	
			/* Started a listen server and the loaded the lobby level | $(PATH_TO_FILE)?$(OPTION) | */
	...
	for (auto Result : SessionSearch->SearchResults)
	{
		...
		if (GEngine)
		{
			...
			GEngine->AddOnScreenDebugMessage(
				...
				10.f,
				FColor::Blue,
				FString::Printf(TEXT("User = %s"), *User)
			);
		}
